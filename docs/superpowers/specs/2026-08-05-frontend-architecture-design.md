...
No calculation logic (average cost, profit) runs in the frontend — the backend is the sole source of truth for those; composables only validate shape/presence of input before sending it. `usePurchaseForm` computes and displays a client-side **subtotal preview** (sum of `quantidade * preco_unitario` across line items, plain arithmetic with no rounding ambiguity) so the user sees a running total while building the item list — this is a UI convenience, not a source of truth; it's discarded and replaced by the backend's response (`total`) once submitted. The sale form shows total and **lucro estimado** live too, but never computes them client-side: it calls a backend preview endpoint (`POST /api/vendas/preview`) that reuses `ProfitCalculatorService` with each product's current `custo_medio` and returns `{ total, lucro, itens }` — read-only, no persistence, no stock mutation. Profit therefore stays authoritative on the backend, and the client holds no arithmetic that could drift from the backend's `custo_medio`.
...
- **Vendas** (`SalesView`): same repeatable line-item form for `POST /vendas`; a live **total + lucro estimado** readout is fetched from `POST /vendas/preview` as the line items change (never computed client-side), and the real total/lucro from the submit response is toasted on success; history table with a "Cancelar" action per row calling `POST /vendas/{id}/cancelar`, disabled for already-cancelled sales. Same as purchases, a successful sale or cancellation refetches `useProductStore` so `estoque` stays current on the Produtos screen.
