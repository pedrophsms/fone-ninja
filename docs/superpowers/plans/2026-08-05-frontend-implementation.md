...
- **Dockerized stack (challenge README §19, added post-plan)**: the frontend repo now ships a `Dockerfile` (two-stage: `node:22-alpine` `npm ci` + build with `VITE_API_BASE_URL=/api`, then `nginx:1.27-alpine` serving the SPA with a `location /api/` reverse proxy to the `backend` service), plus `docker-compose.yml` orchestrating frontend + backend (built from sibling `../fone-ninja-backend`) + MySQL, all internal except the frontend on `FRONTEND_PORT` (8080). `APP_KEY` is required via `.env.docker.example` (fail-fast with `${APP_KEY:?...}`). Same-origin through nginx, so no CORS in production. Verified end-to-end: images build, `migrate --seed` runs, SPA + `/login` fallback serve, and `/api/login` + `/api/produtos` respond through the proxy.
