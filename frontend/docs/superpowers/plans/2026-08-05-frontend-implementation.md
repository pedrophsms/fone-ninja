...
- **Dockerized stack (challenge README §19, added post-plan)**: the frontend repo ships a two-stage `Dockerfile` (`node:22-alpine` `npm ci` + build with `VITE_API_BASE_URL=/api`, then `nginx:1.27-alpine` serving the SPA with a `location /api/` reverse proxy to the `backend` service) and `nginx.conf`. The root `docker-compose.yml` orchestrating frontend + backend + MySQL lives in a **dedicated root directory `fone-ninja-compose/`** (sibling of both repos, matching the challenge's `/projeto` layout: compose at the root, `frontend/` and `backend/` each with their own Dockerfile), with build contexts `../fone-ninja-frontend` and `../fone-ninja-backend`. Only the frontend is published (`FRONTEND_PORT` 8080); backend/mysql are internal. `APP_KEY` is required via `.env.docker.example` (fail-fast `${APP_KEY:?...}`). Same-origin through nginx, so no CORS in production. Verified end-to-end: images build, `migrate --seed` runs, SPA + `/login` fallback serve, and `/api/login` + `/api/produtos` respond through the proxy.
